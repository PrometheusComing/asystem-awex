...
from awex.transfer.nccl_comm import execute_tensors_to_copy, \
    validate_rank_mappings, detect_hang
...
class NcclColocateStreamBatchTransport:
    ...
    def execute_recursive_partition_stream_transfer(
        ...
        logger.info(f"[{os.getpid()}] [{rank_coordinate}]  Starting recursive partition transfer with {num_rounds} rounds")
        ...
        logger.info(f"[{os.getpid()}] [{rank_coordinate}] All {num_rounds} rounds completed")
